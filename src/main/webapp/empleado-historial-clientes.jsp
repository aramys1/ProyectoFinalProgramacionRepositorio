...
<head>
    ...
    <title>Historial por cliente | Rewind &amp; Relive</title>
</head>
...
<body>
    ...
    <main class="employee-page">
        <section class="employee-header">
            <p class="employee-kicker">Historial.clientes</p>
            <h1>Historial por cliente</h1>
            <p>Selecciona un cliente para consultar todos sus alquileres.</p><br><a class="retro-button" href="empleado-alquileres.jsp">VOLVER A ALQUILERES</a>
        </section>
        <section class="retro-window employee-panel">
            <div class="window-header"><span>Clientes.table</span><span>_ [] X</span></div>
...
%><tr>
                            ...
                            <td><a class="employee-mini-button" href="empleado-historial-clientes.jsp?cliente=<%= rs.getInt("id_usuario") %>">Ver historial</a></td>
                        </tr><% }
        if(!hay){ %><tr>
                            <td colspan="5" class="employee-empty">No hay clientes registrados.</td>
                        </tr><% }
    } catch(SQLException e){ %><tr>
                            <td colspan="5">No fue posible cargar los clientes: <%= h(e.getMessage()) %></td>
                        </tr><% } %>
                    </tbody>
                </table>
            </div>
        ...
        <% if(idCliente > 0) { %>
        <section class="retro-window employee-panel">
            <div class="window-header"><span>Historial_Cliente_<%= idCliente %>.table</span><span>_ [] X</span></div>
            <form class="employee-toolbar" method="get"><input type="hidden" name="cliente" value="<%= idCliente %>">
                ...
                <button class="retro-button" type="submit">FILTRAR</button><a class="employee-clear" href="empleado-historial-clientes.jsp?cliente=<%= idCliente %>">Limpiar</a>
            </form>
            <div class="employee-table-wrap">
                <table class="employee-table">
                    <thead>
...
%><tr>
        ...
        if(!hay){%><tr>
                            <td colspan="7" class="employee-empty">No se encontraron alquileres para este cliente.</td>
                        </tr><% }}
    }catch(SQLException e){%><tr>
