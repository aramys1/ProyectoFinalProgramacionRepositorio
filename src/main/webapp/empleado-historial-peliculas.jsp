...
<head>
    ...
    <title>Historial por pelicula | Rewind &amp; Relive</title>
</head>
...
<body>
    ...
    <main class="employee-page">
        <section class="employee-header">
            <p class="employee-kicker">Historial.peliculas</p>
            <h1>Historial por pelicula</h1>
            <p>Selecciona una pelicula para consultar todos sus alquileres.</p><br><a class="retro-button" href="empleado-alquileres.jsp">VOLVER A ALQUILERES</a>
        </section>
        <section class="retro-window employee-panel">
            <div class="window-header"><span>Peliculas.table</span><span>_ [] X</span></div>
            <form class="employee-toolbar" method="get">
                ...
                <button class="retro-button" type="submit">FILTRAR</button><a class="employee-clear" href="empleado-historial-peliculas.jsp">Limpiar</a>
            </form>
...
%><tr>
                            ...
                            <td><%= rs.getInt("copias") %></td>
                            <td><%= rs.getInt("total") %></td>
                            <td><a class="employee-mini-button" href="empleado-historial-peliculas.jsp?pelicula=<%= rs.getInt("id_pelicula") %>">Ver historial</a></td>
                        </tr><% }
        if(!hay){%><tr>
                            <td colspan="5" class="employee-empty">No se encontraron peliculas.</td>
                        </tr><%}}
    }catch(SQLException e){%><tr>
                            <td colspan="5">No fue posible cargar las peliculas: <%= h(e.getMessage()) %></td>
                        </tr><%}%>
</tbody></table></div></section>
<% if(idPelicula>0){ %>
                        <section class="retro-window employee-panel">
                            <div class="window-header"><span>Historial_Pelicula_<%= idPelicula %>.table</span><span>_ [] X</span></div>
                            <form class="employee-toolbar" method="get"><input type="hidden" name="pelicula" value="<%= idPelicula %>">
                                ...
                                <button class="retro-button" type="submit">FILTRAR</button><a class="employee-clear" href="empleado-historial-peliculas.jsp?pelicula=<%= idPelicula %>">Limpiar</a>
                            </form>
                            <div class="employee-table-wrap">
                                <table class="employee-table">
                                    <thead>
...
%><tr>
                                            ...
                                            <td><span class="status-badge <%= devolucion==null?"status-warning":"status-ok" %>"><%= h(estado) %></span></td>
                                        </tr><%}
        if(!hay){%><tr>
                                            <td colspan="7" class="employee-empty">No se encontraron alquileres para esta pelicula.</td>
                                        </tr><%}}
    }catch(SQLException e){%><tr>
                                            <td colspan="7">No fue posible cargar el historial: <%= h(e.getMessage()) %></td>
                                        </tr><%}%>
</tbody></table></div></section><% } %>
