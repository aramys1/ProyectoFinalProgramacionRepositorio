...
function initHeroPosterSlider() {
  ...
  const posters = (poster.dataset.posters || "")
    .split(",")
    .map(function (name) {
      return name.trim();
    })
...
function initInventoryTemplate() {
  ...
  function selectMovie(button, shouldScroll) {
    ...
    if (fields.available)
      fields.available.textContent = button.dataset.available || "0";
    if (fields.rented) fields.rented.textContent = button.dataset.rented || "0";
    if (fields.damaged)
      fields.damaged.textContent = button.dataset.damaged || "0";
    if (fields.poster && button.dataset.poster) {
      fields.poster.src = button.dataset.poster;
      fields.poster.alt =
        "Poster de " + (button.dataset.title || "pelicula seleccionada");
    }
...
function buscarUsuario() {
  ...
  if (buscar === "") {
    filas.forEach(function (fila) {
      fila.style.display =
        fila.getAttribute("data-rol") === "EMPLEADO" ? "" : "none";
    });
    return;
  }
  ...
  filas.forEach(function (fila) {
    ...
    fila.style.display =
      nombre.includes(buscar) || cedula.includes(buscar) ? "" : "none";
  });
}
